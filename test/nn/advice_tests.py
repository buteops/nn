...
def model_test(target, classes, input_size):
  ...
  assert target.input.shape.as_list() == [None, input_size], f"Wrong input shape. Expected [None,  {input_size}] but got {target.input.shape.as_list()}"
  i = 0
  expected = [[Dense, [None, 120], relu], [Dense, [None, 40], relu], [Dense, [None, classes], linear]]

  for layer in target.layers:
    assert type(layer) == expected[i][0], f"Wrong type in layer {i}. Expected {expected[i][0]} but got {type(layer)}"
    assert layer.output.shape.as_list() == expected[i][1], f"Wrong number of units in layer {i}. Expected {expected[i][1]} but got {layer.output.shape.as_list()}"
    assert layer.activation == expected[i][2], f"Wrong activation in layer {i}. Expected {expected[i][2]} but got {layer.activation}"
...
def model_s_test(target, classes, input_size):
  ...
  assert len(target.layers) == 2, f"Wrong number of layers. Expected 3 but got {len(target.layers)}"
  assert target.input.shape.as_list() == [None, input_size], f"Wrong input shape. Expected [None,  {input_size}] but got {target.input.shape.as_list()}"
  i = 0
  expected = [[Dense, [None, 6], relu], [Dense, [None, classes], linear]]
...
def model_r_test(target, classes, input_size):
  ...
  i = 0
  expected = [
    [Dense, [None, 120], relu, (tf.keras.regularizers.l2, 0.1)],
